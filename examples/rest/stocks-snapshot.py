...
def print_snapshots(iterator: Iterator[UniversalSnapshot]):
...
it = client.list_asset_snapshots(
    ticker_any_of=["AAPL", "O:AAPL230519C00055000", "DOES_NOT_EXIST", "X:1INCHUSD", "C:AEDAUD"]
)
print_snapshots(it)

it = client.list_asset_snapshots(market_type=SnapshotMarketType.STOCKS, ticker_gt="A", ticker_lt="AAPL")
print_snapshots(it)

it = client.list_asset_snapshots(market_type=SnapshotMarketType.STOCKS, ticker_gte="AAPL", ticker_lte="ABB")
print_snapshots(it)
