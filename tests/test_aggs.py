...
from polygon.rest.models import Agg, GroupedDailyAgg, DailyOpenCloseAgg, PreviousCloseAgg
...
class AggsTest(BaseTest):
    ...
    def test_get_grouped_daily_aggs(self):
        ...
        expected = [GroupedDailyAgg(ticker="GIK", open=9.99, high=10.02, low=9.9, close=10.02, volume=895345, vwap=9.9979, timestamp=1602705600000, transactions=96)]
        self.assertEqual(aggs, expected)
    ...
    def test_get_previous_close_agg(self):
        ...
        expected = [PreviousCloseAgg(ticker='AAPL', close=156.8, high=162.34, low=156.72, open=162.25, timestamp=1651003200000, volume=95595226.0, vwap=158.6074)]
        self.assertEqual(aggs, expected)